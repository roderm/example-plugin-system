...
func (i *Issue) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     i.ID,
		Type:   "Issue",
		Fields: make([]*Field, 3),
	...
	if buf, err = json.Marshal(i.IsPr); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "bool",
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case issue.Table:
		query := c.Issue.Query().
			Where(issue.ID(id))
		query, err := query.CollectFields(ctx, "Issue")
		if err != nil {
			return nil, err
		}
